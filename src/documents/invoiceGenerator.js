...
export async function generateInvoicePDF({
  ...
  doc
    ...
    .setFontSize(16)
    .setTextColor(...orange);
  doc.text("OVERSTAY", pageWidth - margin, y + 4, { align: "right" });
  doc.text("INVOICE", pageWidth - margin, y + 11, { align: "right" });
  ...
  doc.text("Cargo Overstay Charges", margin + idxColW + 2, y);

  doc
    .setFont("helvetica", "normal")
    .setFontSize(7.5)
    .setTextColor(41, 128, 185);
  doc.text(`${category} - ${subcategory}`, margin + idxColW + 2, y + 4.5);
  doc.text(
    `${overstayDays ?? 0} day(s) x ${Number(baseRate ?? 0).toFixed(2)} AED`,
    margin + idxColW + 2,
    y + 8.4,
  );
  ...
  doc.text(
    `${Number(subtotal ?? 0).toFixed(2)} AED`,
    pageWidth - margin - 2,
    y,
    {
      align: "right",
  ...
  y += 5;
  doc.text(`Tax (${Number(taxPercent ?? 0)}%)`, margin, y);
  doc.text(
    `${(Number(total ?? 0) - Number(subtotal ?? 0)).toFixed(2)} AED`,
    pageWidth - margin,
    y,
    { align: "right" },
  );
  ...
  doc.text(`${Number(total ?? 0).toFixed(2)} AED`, pageWidth - margin, y, {
    align: "right",
  });
  y += 12;
