                            ...
                            <AccordionDetails sx={{ p: 3, bgcolor: "#fff" }}>
                                <Stack spacing={2}>
                                    ...
                                    {(() => {
                                        ...
                                        const removeContainerDetailFromDb = async (item, containerDetail, receiverId) => {
                                            ...
                                            if (!orderId || !receiverId || !orderItemId || !cid || qty <= 0) {
                                                return;
                                            }
                                            ...
                                            if (!res.data?.success) {
                                                throw new Error(res.data?.details || res.data?.error || 'Container remove failed');
                                            }
                                        };
                                        ...
                                        const removeSenderContainerDetail = async (index, shippingIndex, containerIndex) => {
                                            const sender = formData.senders[index];
                                            const shippingDetail = sender?.shippingDetails?.[shippingIndex];
                                            const containerDetail = shippingDetail?.containerDetails?.[containerIndex];

                                            if (!sender || !shippingDetail || !containerDetail) return;

                                            const ok = window.confirm('Remove this container assignment?');
                                            if (!ok) return;

                                            try {
                                                await removeContainerDetailFromDb(shippingDetail, containerDetail, sender.id);
                                                ...
                                                console.error('[removeSenderContainerDetail] Error:', err.response?.data || err.message);

                                                setSnackbar({
                                                    open: true,
                                                    message: err.response?.data?.details || err.response?.data?.error || err.message || 'Failed to remove container',
                                                    severity: 'error',
                                                });
                                            }
                                        };
                                        ...
                                        const removeReceiverContainerDetail = async (index, shippingIndex, containerIndex) => {
                                            const receiver = formData.receivers[index];
                                            const shippingDetail = receiver?.shippingDetails?.[shippingIndex];
                                            const containerDetail = shippingDetail?.containerDetails?.[containerIndex];

                                            if (!receiver || !shippingDetail || !containerDetail) return;

                                            const ok = window.confirm('Remove this container assignment?');
                                            if (!ok) return;

                                            try {
                                                await removeContainerDetailFromDb(shippingDetail, containerDetail, receiver.id);
                                                ...
                                                console.error('[removeReceiverContainerDetail] Error:', err.response?.data || err.message);
                                        ...
                                        const renderRecForm = (rec, i) => {
                                            ...
                                            const removeContainerDetail = async (shippingIndex, containerIndex) => {
                                ...
                                <Stack spacing={2}>
                                    ...
                                    {formData.receivers.some(rec => rec.containers && rec.containers.length > 0) && (
                                        ...
                                        <Stack spacing={1}>
                                            ...
                                            {console.log('Assigned Containers Data:', formData.receivers?.flatMap(rec => rec.containers || []))}
                                        </Stack>
                                    )}
                                    {/* New: Global Totals */}
                                    <Divider />
