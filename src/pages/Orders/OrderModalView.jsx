...
const OrderModalView = ({
  ...
  const fetchContainers = async () => {
    ...
    try {
      ...
      setAssignmentError(
        'Failed to fetch containers. Please check backend query for table "cm".',
      );
      setSnackbar({
        open: true,
  ...
  const handleAssignContainer = async (receiverId, containerId) => {
    ...
    try {
      await api.post(
        `/api/orders/${selectedOrder.id}/receivers/${receiverId}/assign-container`,
      ...
      setAssignmentError(
        err.response?.data?.error || "Failed to assign container",
      );
      setSnackbar({
        open: true,
        message: "Failed to assign container",
        severity: "error",
      });
    }
  };
    ...
    if (order.receivers?.length) {
      ...
      for (const receiver of order.receivers) {
        ...
        receiver.shippingdetails.forEach((shippingDetail, detailIndex) => {
          ...
          doc.setTextColor(...brandPrimary);
          doc.text(
            `Container Assignments for Item ${detailIndex + 1}: ${shippingDetail.itemRef}`,
            margin,
            y,
          );
          y += 4;
  ...
  const generatePDFWithCanvas = async () => {
        ...
        <div class="section">
            ...
            <div class="section-content">
                <table>
                    ...
                    <tr>
                        <th>Created At</th>
                        <td>${new Date(selectedOrder.created_at).toLocaleString(
                          "en-US",
                          {
                            year: "numeric",
                            month: "short",
                  ...
                  .map((rec, index) => {
                    ...
                    ${
                      rec.shippingdetails && rec.shippingdetails.length > 0
                        ? ` <!-- Note: Fixed property name to 'shippingdetails' as per JSON -->
                      ...
                      .map(
                        ...
                        <div style="margin-bottom: 15px; padding: 10px; background: #fafafa; border-radius: 4px;">
                            ...
                            <table class="container-table">
                                ...
                                <tbody>
                                    ${item.containerDetails
                                      .map((contDetail, contIndex) => {
                                        const container =
                                          contDetail.container || {};
                                        return `
                                        <tr>
    ...
    // Save the PDF
    pdf.save(
      `Order_${selectedOrder.booking_ref || "Unknown"}_${Date.now()}.pdf`,
    );
  };
        ...
        {files.map((file, index) => (
          ...
          >
            ...
            <ListItemText
              ...
              secondary={`${file?.mimetype || "Unknown"} • ${(
                (file?.size || 0) / 1024
              ).toFixed(1)} KB`}
            />
          </ListItem>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <TableContainer sx={{ mb: 3 }}>
                <Table>
                  ...
                  <TableBody>
                    {receivers.map((rec) => {
                      const normContainers = normalizeContainers(
                        rec.containers,
                      );
                      return (
                  ...
                  <Grid item xs={12} md={6} key={rec.id || rec.receiver_name}>
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleAssignContainer(rec.id, assignments[rec.id])
                        }
                        disabled={!assignments[rec.id]}
                      >
                        Assign
                          ...
                          <TableRow key={container.id}>
                            ...
                            <TableCell>
                              {container.owner_type === "soc"
                                ? container.available_at
                                : container.place_of_loading}
                            </TableCell>
                            <TableCell>
                              {container.owner_type === "soc"
                                ? container.purchase_from
                    ...
                    <Grid item xs={12}>
                      <Card
                        sx={{
                          ...
                          transition: "all 0.3s ease",
                          "&:hover": {
                            boxShadow: "0 8px 25px rgba(245, 130, 32, 0.1)",
                          },
                        }}
                      >
                        <CardContent sx={{ p: 3 }}>
                          ...
                          <HorizontalKeyValue
                            data={{
                              Status:
                                ...
                                "Created",
                              "RGL Booking Number":
                                selectedOrder?.rgl_booking_number,
                              ...
                              "Consignment Marks":
                                selectedOrder?.consignment_marks,
                      ...
                      >
                        <CardContent sx={{ p: 3 }}>
                          ...
                          >
                            ...
                            >
                              Receivers (
                              {(selectedOrder?.receivers || []).length})
                            </Typography>
                          </Box>
                          {renderReceivers()}
                        </CardContent>
                  ...
                  <Grid container spacing={3}>
                    ...
                    {selectedOrder?.transport_type === "Drop Off" && (
                      <Grid item xs={12}>
                              ...
                              >
                                <CardContent sx={{ p: 3 }}>
                                  ...
                                  <Typography
                                    ...
                                    sx={{ mb: 2 }}
                                  >
                                    {dropOffs.length} Drop-Off Entr
                                    {dropOffs.length > 1 ? "ies" : "y"}
                                  </Typography>
                        ...
                        {selectedOrder?.receivers?.every(
                          (r) => !(r.drop_off_details || []).length,
                        ) && (
                          <Alert severity="info" sx={{ mt: 2 }}>
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                      ...
                      >
                        ...
                        >
                          <AttachFileIcon sx={{ color: "#f58220" }} />
                          <Typography fontWeight={700}>Gatepass</Typography>
