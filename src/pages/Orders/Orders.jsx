...
const OrdersList = ({ 
  ...
  // Simplified: Use main states for standalone, props for controlled
  const [selectedOrders, setSelectedOrders] = useState(propSelectedOrders.length > 0 ? propSelectedOrders : []);
  const [orders, setOrders] = useState(propOrders.length > 0 ? propOrders : []);
  const [filters, setFilters] = useState({ ...propFilters, status: "", booking_ref: "" });
  ...
  // Toggle handler: Supports both controlled (calls propOnToggle) and uncontrolled (local state) modes
  const handleToggle = useCallback((id) => {
    console.log('Toggling order ID:', id); // Clean log: Only show the ID
    if (propOnToggle) {
      // Controlled: Notify parent to toggle (parent handles state update)
      propOnToggle(id);
    } else {
      // Uncontrolled: Toggle local state
  ...
  // Update local states if props change (for controlled mode syncing)
  useEffect(() => {
    if (propSelectedOrders.length > 0) {
      setSelectedOrders(propSelectedOrders);
    }
  }, []);
...
//       setFilters(prev => ({ ...prev, ...propFilters }));
//     }
//   }, [propFilters]);
...
//   }, [propLoading]);
  ...
  const handleSelectAllClick = useCallback((event) => {
    if (event.target.checked) {
      ...
      if (propOnToggle) {
        newSelected.forEach(id => propOnToggle(id));
      } else {
    ...
    if (propOnToggle) {
      selectedOrders.forEach(id => propOnToggle(id));
    } else {
      setSelectedOrders([]);
    }
  }, [orders, selectedOrders, propOnToggle]);

  const isSelected = useCallback((id) => selectedOrders.indexOf(id) !== -1, [selectedOrders]);

  const handleClick = useCallback((id) => {
    console.log('Row clicked, toggling order ID:', id);
    handleToggle(id);
  }, [handleToggle]);
  ...
  }, [selectedOrders, propOnToggle]);

  // Fetch orders only if standalone (no propOrders)
  const fetchOrders = useCallback(async () => {
    if (propOrders.length > 0) return; // Skip if controlled
    setLoading(true);
    setError(null);
    try {
      const params = {
  ...
  }, [page, rowsPerPage, filters, propOrders]);
  ...
  const handleAssign = async (containerId, quantities) => {
      ...
      try {
          ...
          clearSelection(); // Clear selection after assign
                                  ...
                                  >
                                      ...
                                      <StyledTableCell>
                                          <Stack direction="row" spacing={1}>
                                              <IconButton size="small" color="#f58220" onClick={(e) => { e.stopPropagation(); handleStatusUpdate(order.id, order); }} title="Update Status">
                                                  <UpdateIcon sx={{ color: '#f58220' }} />
                                              </IconButton>
                                              <IconButton size="small" color="#f58220" onClick={(e) =>  { e.stopPropagation(); handleView(order.id); }} title="View Details">
                                                  <VisibilityIcon sx={{ color: '#f58220' }} />
                                              </IconButton>
                                              <IconButton size="small" color="#f58220" onClick={(e) => { e.stopPropagation(); handleEdit(order.id); }} title="Edit">
                                                  <EditIcon sx={{ color: '#f58220' }} />
                                              </IconButton>
                                          </Stack>
                                      </StyledTableCell>
                                  </StyledTableRow>
