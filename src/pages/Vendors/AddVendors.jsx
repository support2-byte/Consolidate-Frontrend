...
export default function VendorForm({ mode = "add" }) {
  ...
  // Other useEffect for fetchvendor (unchanged, omitting documents)
  useEffect(() => {
    if (mode === 'edit' && id) {
      const fetchvendor = async () => {
        try {
          ...
          showToast('Failed to fetch vendor data', 'error');
        }
      };
      fetchvendor();
    }
  ...
  const handleSaveContacts = async () => {
    try {
      if (!id) {
        showToast("Cannot save contacts: vendor ID is missing", "error");
        return;
      }
  ...
  const handleAddDocument = async () => {
    if (!id) {
      showToast('Cannot upload document: vendor ID is missing', 'error');
      return;
    }
  ...
  const handleDeleteDocument = async (document_id) => {
    try {
      await api.delete(`/api/vendors/${id}/documents/${document_id}`);
  ...
  const handleSavevendor = async () => {
    if (!form.contact_name || !form.email) {
      showToast("vendor name and email are required", "error");
      return;
    }
    try {
      if (mode === "edit") {
  ...
  const docColumns = [
    {
      ...
      renderCell: (params) => (
        <a
          href={`/api/vendors/${id}/documents/${params.row.document_id}/download`}
...
<Box sx={{ display: "flex", gap: 2, mt: 2 }}> 
            <TextField
              ...
              helperText={!form.contact_name ? "vendor name is required" : ""}
            />
